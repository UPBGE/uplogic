from bgui.bgui_utils import Layout as Layout
from bgui.bgui_utils import System
from bgui import Frame
from uplogic.data import GlobalDB
import bge
import bgui


def get_ui_system(name: str = 'default', theme: str = '') -> System:
    ...
    if ui_systems.check(name) is None:
        ui_systems.put(name, System(theme))
    return ui_systems.get(name)


class ULLayout(Layout):
    def __init__(self, system: str = 'default', factory=Layout):
        self.system = get_ui_system(system)
        if self.system.layout is None:
            self.system.load_layout(factory, None)
...
class SimpleLayout(Layout):
    ...
    def __init__(self, sys, data):
        ...
        self.frame = Frame(self, border=0)
        self.frame.colors = [(0, 0, 0, 0) for i in range(4)]

        # A themed frame
        self.win = Frame(self, size=[0.6, 0.8],
        options=bgui.BGUI_DEFAULT|bgui.BGUI_CENTERED)
        
        # Create an image to display
        self.win.img = bgui.Image(self.win, 'E:\\Test\\img.jpg', size=[.92, .7], pos=[.01, .24],
        ...
        self.progress = bgui.ProgressBar(self.win, percent=0.0, size=[0.92, 0.06], pos=[.2, 0.17], sub_theme="Progress", options=bgui.BGUI_DEFAULT | bgui.BGUI_CENTERX)
        self.health = bgui.ProgressBar(self.win, percent=0.5, size=[0.92, 0.02], pos=[0, 0.14], sub_theme="Health",  options=bgui.BGUI_DEFAULT|bgui.BGUI_CENTERX)
        
        # A few TextInput widgets
        self.input = bgui.TextInput(self.win, text="I'm active.", font="E:\\Test\\myfont.otf", size=[.4, .04], pos=[.04, 0.02],
        ...
        bge.logic.getCurrentScene().post_draw.append(self._draw) #there was post_draw.append(self.render) but it works with _draw
...
def main(cont):
    ...
    if 'sys' not in own:
        ...
        own['sys'] = System('E:\\Test\\bgui_themes\\default')
