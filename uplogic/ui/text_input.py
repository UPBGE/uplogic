...
class TextInput(Widget):
    ...
    def __init__(self, parent, name=None, text="", prefix="", font=None, pt_size=None, color=None,
        ...
        Widget.__init__(self, parent, name, aspect,
                        size, pos, sub_theme, options)
        ...
        # create widgets
        self.frame = Frame(
            self, size=[1, 1], options=BGUI_NO_FOCUS | BGUI_DEFAULT | BGUI_CENTERY)
        self.highlight = Frame(self, size=self.frame.size, border=0,
                               options=BGUI_NO_FOCUS | BGUI_CENTERY | BGUI_NO_NORMALIZE)
        self.cursor = Frame(self, size=[
                            1, 1], border=0, options=BGUI_NO_FOCUS | BGUI_CENTERY | BGUI_NO_NORMALIZE)
        self.label = Label(self, text=text, font=font, pt_size=pt_size,
                           sub_theme=self.theme['LabelSubTheme'], options=BGUI_NO_FOCUS | BGUI_DEFAULT)
        ...
        self.fd = blf.dimensions(self.label.fontid, self.text_prefix)[
            0] + fd[1] / 3.2
    ...
    def _update_char_widths(self):
        ...
        for char in self.text:
            self.char_widths.append(blf.dimensions(
                self.label.fontid, char * 20)[0] / 20)
    ...
    def update_selection(self):
        left = self.fd + \
            blf.dimensions(self.label.fontid, self.text[:self.slice[0]])[0]
        right = self.fd + \
            blf.dimensions(self.label.fontid, self.text[:self.slice[1]])[0]
    ...
    def _handle_key(self, key, is_shifted):
        ...
        if key == BACKSPACEKEY:
            if slice_len != 0:
                self.label.text = self.text[:self.slice[0]
                                            ] + self.text[self.slice[1]:]
                self.char_widths = self.char_widths[:self.slice[0]
                                                    ] + self.char_widths[self.slice[1]:]
                self.slice = [self.slice[0], self.slice[0]]
                # handle char length list
            elif self.slice[0] > 0:
                self.label.text = self.text[:self.slice[0] -
                                            1] + self.text[self.slice[1]:]
            ...
            if ord(AKEY) <= key <= ord(ZKEY):
                ...
                char = "."
            elif key == PADSLASHKEY:
                char = "/"
            elif key == PADASTERKEY:
                char = "*"
            elif key == PADMINUS:
                char = "-"
            elif key == PADPLUSKEY:
            ...
            # elif key == TABKEY: char = "\t"
                ...
                if key == ACCENTGRAVEKEY:
                    char = "`"
                elif key == MINUSKEY:
                    char = "-"
                elif key == EQUALKEY:
                    char = "="
                elif key == LEFTBRACKETKEY:
                    char = "["
                elif key == RIGHTBRACKETKEY:
                    char = "]"
                elif key == BACKSLASHKEY:
                    char = "\\"
                elif key == SEMICOLONKEY:
                    char = ";"
                elif key == QUOTEKEY:
                    char = "'"
                elif key == COMMAKEY:
            ...
            if char:
                ...
                self.label.text = self.text[:self.slice[0]
                                            ] + char + self.text[self.slice[1]:]
                self.char_widths = self.char_widths[:self.slice[0]] + [blf.dimensions(
                    self.label.fontid, char * 20)[0] / 20] + self.char_widths[self.slice[1]:]
                self.slice = [self.slice[0] + 1, self.slice[0] + 1]
                self.slice_direction = 0
