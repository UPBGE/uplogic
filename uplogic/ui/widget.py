...
class Animation:
    ...
    def update(self):
        ...
        setattr(self.widget, self.attrib, getattr(
            self.widget, self.attrib) + dv)
...
class Widget:
    ...
    def __init__(self, aspect=None, size=[1, 1], pos=[0, 0], sub_theme='',
    ...
    def _generate_theme(self):
        if isinstance(self.theme_options, set):
            ...
            theme = theme[self.theme_section] if theme.has_section(
                self.theme_section) else None
    ...
    def move(self, position, time, callback=None):
        ...
        self.anims.append(ArrayAnimation(
            self, "position", position, time, callback))
    ...
    def add_widget(self, widget):
        ...
        if widget in self.children:
            raise ValueError(
                "%s is already attached to this widget" % (widget))
