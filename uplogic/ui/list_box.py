...
class ListBox(Widget):
    ...
    def __init__(self, parent, name=None, items=[], padding=0, aspect=None, size=[1, 1], pos=[0, 0], sub_theme='', options=BGUI_DEFAULT):
        ...
        Widget.__init__(self, parent, name, aspect=aspect,
                        size=size, pos=pos, sub_theme='', options=options)

        self._items = items
        #: The amount of extra spacing to put between items
        self.padding = padding if padding is not None else self.theme['Padding']

        self.highlight = Frame(self, "frame", border=1,
                               size=[1, 1], pos=[0, 0])
    ...
    def _draw(self):
        ...
        for idx, item in enumerate(self.items):
            ...
            w.position = [
                0, 1 - (idx + 1) * (w.size[1] / self.size[1]) - (idx * self.padding)]
            w.size = [1, w.size[1] / self.size[1]]
            self._spatial_map[item] = [i[:]
                                       for i in w.gpu_view_position]  # Make a full copy
            w._draw()

            if self.selected == item:
                self.highlight.gpu_view_position = [
                    i[:] for i in w.gpu_view_position]
